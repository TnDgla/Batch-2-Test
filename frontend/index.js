document.addEventListener("DOMContentLoaded", async () => {
  try {
    ...
    searchBar.addEventListener("input", (e) => {
      ...
      const f = data.filter(
        (entry) =>
          ...
          entry.roll.includes(term) ||
          (entry.section && entry.section.toLowerCase().includes(term))
      );
      renderLeaderboard(f);
    });
    ...
    //     const ddd = document.getElementById("div1");
    //     ddd.appendChild(data23);
    // });
    

    // const showDivsBtn = document.getElementById('show-divs-btn');
    // const divContainer = document.getElementById('div-container');
    // showDivsBtn.addEventListener('click', () => {
